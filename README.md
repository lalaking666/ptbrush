...
4. **打开 Web 界面填配置**
   ...
   - **qBittorrent 下载器**：WebUI 地址、用户名、密码（旁边有"测试连接"按钮）
   - **PT 站点**：站点名（如 `M-Team`）、Cookie / Header（M-Team 在此填 `x-api-key`）

   填完点右上角 **"保存"**，配置会自动写回 `data/config.toml`，最长 15 分钟（一个 cron 周期）后自动生效，**无需重启容器**。
...
| 测试 QB 连接 | 配置页 → "qBittorrent 下载器" 头部 | 不写盘，仅试连一次返回成功/失败原因 |
| 测试站点 | 配置页 → 每个站点卡片头部 | 尝试拉取 5 条 Free 种子验证 cookie / API key |
| 导出配置 | 配置页 → 顶部"导出" | 下载当前配置为 toml 文件（**不含**登录密码 / secret_key） |
| 导入配置 | 配置页 → 顶部"导入" | 上传 toml 文件覆盖当前配置；当前 toml 自动备份到 `config.toml.bak`；登录密码本地值保留 |
